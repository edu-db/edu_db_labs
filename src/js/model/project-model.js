...
Project.assignUser = (project_id, user_id, role, result) => {
    // Check if project exists.
        ...
        if (res.length == 0) {
            ...
            db.query(`SELECT * FROM users WHERE id=${user_id}`, (err, res) => {
                ...
                if (res.length == 0) {
                    ...
                    db.query(`SELECT user_id FROM roles WHERE project_id=${project_id} AND user_id=${user_id}`, (err, res) => {
                        if (err) {
                            console.log('Something went wrong');
                            result(null, { status: false, message: err });
                        }
                        if (res.length > 0) {
                            ...
                            db.query(`INSERT INTO roles (project_id, user_id, role) VALUES (${project_id}, ${user_id}, '${role}')`, (err, res) => {
                                if (err) {
                                    console.log('Something went wrong');
                                    result(null, { status: false, message: err });
                                } else {
